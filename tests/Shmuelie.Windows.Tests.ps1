...
Describe 'Windows supported module boundary' {
    It 'loads the source module without importing experimental behavior' {
        $output = & pwsh -NoProfile -NonInteractive -File (Join-Path $PSScriptRoot 'fixtures' 'Test-WindowsModuleBoundary.ps1') `
            -ManifestPath (Join-Path $repoRoot 'modules' 'Shmuelie.Windows' 'Shmuelie.Windows.psd1')
        $LASTEXITCODE | Should -Be 0
        $output | Should -Contain 'Windows supported boundary passed.'
    }

    It 'ignores stale experimental binaries and help in a source-like module' {
        $copy = Join-Path $TestDrive 'stale-source' 'Shmuelie.Windows'
        New-Item -ItemType Directory -Path (Split-Path $copy -Parent) -Force | Out-Null
        Copy-Item -LiteralPath (Join-Path $repoRoot 'modules' 'Shmuelie.Windows') -Destination $copy -Recurse
        $help = New-Item -ItemType Directory -Path (Join-Path $copy 'bin' 'en-US') -Force
        Set-Content -LiteralPath (Join-Path $copy 'bin' 'Shmuelie.Windows.AppInstall.dll') -Value 'must not be loaded'
        Set-Content -LiteralPath (Join-Path $help.FullName 'Shmuelie.Windows.AppInstall.dll-Help.xml') -Value 'must not be loaded'
        $output = & pwsh -NoProfile -NonInteractive -File (Join-Path $PSScriptRoot 'fixtures' 'Test-WindowsModuleBoundary.ps1') `
            -ManifestPath (Join-Path $copy 'Shmuelie.Windows.psd1')
        $LASTEXITCODE | Should -Be 0
        $output | Should -Contain 'Windows supported boundary passed.'
    }

    It 'packages Windows independently and preserves its supported exports' {
        $buildOutput = @(& (Join-Path $repoRoot 'build' 'Build-Module.ps1') -Module Shmuelie.Windows -OutputPath (Join-Path $TestDrive 'packaged'))
        $artifact = @($buildOutput | Where-Object { $_ -is [IO.DirectoryInfo] })
        $artifact | Should -HaveCount 1
        $output = & pwsh -NoProfile -NonInteractive -File (Join-Path $PSScriptRoot 'fixtures' 'Test-WindowsModuleBoundary.ps1') `
            -ManifestPath (Join-Path $artifact[0].FullName 'Shmuelie.Windows.psd1')
        $LASTEXITCODE | Should -Be 0
        $output | Should -Contain 'Windows supported boundary passed.'
        Test-Path -LiteralPath (Join-Path $artifact[0].FullName 'bin' 'Shmuelie.Windows.AppInstall.dll') | Should -BeFalse
    }
}
