...
`VSCode`, `WinGet`, and `AppInstaller`. The six portable providers and the
Windows-only **AppInstaller** provider are implemented. **WinGet** remains
separate follow-up work and reports explicit `Skipped` results.
