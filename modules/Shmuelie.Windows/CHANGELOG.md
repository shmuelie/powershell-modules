...
- `Update-AppInstallerApp` uses the original App Installer URI with deferred
  ...
  Registration occurs at next activation without forcing applications to close
  or restart. Older supported builds keep the non-forcing legacy operation and
  explicit in-use errors; failed modern requests are not retried through it.
  Default void output and opt-in request-completion results remain unchanged,
  including the aggregate AppInstaller provider's request-only meaning. (#256)
...
- Extract unpublished AppInstallManager commands, help and formatting into a
  ...
  default tests and publication. Existing AppInstaller commands and projection
